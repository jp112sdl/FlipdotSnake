...
uint32_t eepSnakeHighScore = EEPROM_ADDR_SNAKE_HIGH_SCORE;
...
void endGame() {
	...
	uint32_t highScore = eeprom_read_dword(&eepSnakeHighScore);
...
void resetSnake() {
	...
	curDirection = static_cast<t_direction>(getRandomNumber(0, 3));
...
void updateSnakeHighScore() {
	...
	if(snakeScore > highScore) eeprom_update_dword(&eepSnakeHighScore, snakeScore);
}
